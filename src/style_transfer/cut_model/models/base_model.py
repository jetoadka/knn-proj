...
class BaseModel(ABC):
    ...
    def get_current_losses(self):
        ...
        for name in self.loss_names:
            if isinstance(name, str):
                loss_v = getattr(self, 'loss_' + name)
                errors_ret[name] = loss_v.item() if hasattr(loss_v, 'item') else float(loss_v)  # float(...) works for both scalar tensor and float number
        return errors_ret
